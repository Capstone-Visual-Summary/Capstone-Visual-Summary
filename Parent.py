...
class Parent:
		...
		for subclass in self.__class__.__subclasses__():
			...
			if instance.name not in self.children_names: # type: ignore
				...
				if version_num in self.children: # type: ignore
					...
					message_warning = (f'Duplicate version number found, system has automatically assigned {version_num} as the new version number. ' + 
	...
	def run(self, version = -1.0):
		...
		if str(version) in self.children:
			...
			latest_version = max(float(ver) for ver in self.children if 'WIP' not in ver)
